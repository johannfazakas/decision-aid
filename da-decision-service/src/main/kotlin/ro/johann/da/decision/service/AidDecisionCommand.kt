...
class AidDecisionCommand(
  ...
  fun execute(userId: UUID, decisionId: UUID): Decision {
    logger.info("aid decision >> userId = $userId, decisionId = $decisionId")
  ...
  private fun validateAid(decision: Decision) {
    when {
      ...
      !decision.allPropertiesAreSet() -> throw Errors.propertiesAreMissing(decision.id)
      !decision.hasValidCriteriaWeights() -> throw Errors.invalidCriteriaWeights(decision.id)
    }
  }
}
