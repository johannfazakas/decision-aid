Feature: Add alternative
  ...
  Scenario: Add alternative to decision
    ...
    And I set the name "Bosch MFK89" on the add alternative input
    When I add the alternative
    Then the response is 201

    Given I plan to add an alternative
    And I set the name "Electrolux UXI87" on the add alternative input
    When I add the alternative
    Then the response is 201
  ...
  Scenario: Add alternative with invalid name
    ...
    Given I plan to add an alternative
    And I set the name "" on the add alternative input
    When I add the alternative
    Then the response is 400

  Scenario: Add alternative on not found decision
    Given I plan to add an alternative
    And I set the name "Logitech MX50" on the add alternative input
